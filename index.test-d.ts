...
expectType<Promise<string>>(
  abortRace([async (_signal) => "hello", async (_signal) => "world"])
);

// Valid usage with options
expectType<Promise<number>>(
  abortRace([async (_signal) => 42], { signal: new AbortController().signal })
);
...
expectError(abortRace([(_signal: AbortSignal) => "sync"]));
